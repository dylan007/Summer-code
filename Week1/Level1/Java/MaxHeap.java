...
class MaxHeap {
	...
	void heapify(int idx) { //heapifies at that index assuming that both it's subtrees are heaps
		int max = idx;
		if(lchild(idx)<size && arr[lchild(idx)]>arr[max])
			max = lchild(idx);
		if(rchild(idx)<size && arr[rchild(idx)]>arr[max])
			max = rchild(idx);
		if(max!=idx){
			swap(idx,max);
			heapify(max);
	...
	public static void main(String[] args) throws IOException {
		...
		while(t-->0){
			int q = nextInt();
			MaxHeap mh = new MaxHeap(q);//there can be maximum of q insertions
			while(q-- > 0){
				int type = nextInt();
				switch(type) {
